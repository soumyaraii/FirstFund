...
import { getScorecard, startSimulation, submitDecision, undoDecision } from "./services/api";
...
function App() {
  ...
  const handleUndo = async () => {
    if (!simulationData?.sessionId || loading) return;

    setLoading(true);
    setError("");

    try {
      const response = await undoDecision({ sessionId: simulationData.sessionId });

      setSimulationData((previousData) => ({
        ...previousData,
        currentState: response.currentState,
        currentScenario: response.currentScenario,
        trajectory: (previousData.trajectory || []).filter(
          (point) => point.month !== response.month
        ),
      }));

      setPage("simulation");
    } catch (err) {
      setError(err.message || "Unable to go back a month.");
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <>
        ...
        <Simulation
          ...
          onBack={handleUndo}
          canGoBack={(simulationData.currentScenario?.month || 1) > 1}
          loading={loading}
        />
      </>
    );
