...
// ---------------------------------------------------------
// Feature: randomized non-pivotal month order
// Positions 1, 6, 7, 12 are fixed (pivotal/emergency/finale
// scenarios always land there). Positions 2,3,4,5,8,9,10,11
// get shuffled scenario ids from that same set, per session.
// ---------------------------------------------------------
const FIXED_POSITIONS = [1, 6, 7, 12];
const SHUFFLEABLE_POSITIONS = [2, 3, 4, 5, 8, 9, 10, 11];
...
function buildScenarioOrder() {
  ...
  FIXED_POSITIONS.forEach(pos => {
    order[pos - 1] = pos; // scenario id === position for fixed months
  });

  const shuffledIds = shuffleArray(SHUFFLEABLE_POSITIONS);
  SHUFFLEABLE_POSITIONS.forEach((pos, idx) => {
    order[pos - 1] = shuffledIds[idx];
  });

  return order; // order[i] = scenario id shown at position i+1
}

// Resolves "what scenario is shown at journey-position N for this session",
// and stamps `.month` with the session position (not the scenario's fixed id)
// so everything downstream (frontend progress bar, decision endpoint,
// history) keeps working against "where am I in the 12 months" as before.
function getScenarioForPosition(session, position) {
  if (!session.scenarioOrder || position < 1 || position > 12) return null;
  const scenarioId = session.scenarioOrder[position - 1];
  const base = scenarios.find(s => s.id === scenarioId);
  if (!base) return null;
  return { ...base, month: position };
}
// ---------------------------------------------------------
...
app.post('/api/simulation/start', (req, res) => {
  ...
  const session = {
    ...
    mentorPersona,
    month: 1,
    scenarioOrder: buildScenarioOrder(),          // NEW
    financials: {
      monthlyIncome,
      fixedExpenses,
  ...
  res.status(201).json({
    ...
    currentScenario: getScenarioForPosition(session, 1)   // was: scenarios[0]
  });
});
...
app.get('/api/simulation/:sessionId', (req, res) => {
  ...
  const currentScenario = session.month <= 12
    ? getScenarioForPosition(session, session.month)   // was: scenarios.find(s => s.month === session.month)
    : null;
...
const scenario = getScenarioForPosition(session, month) || getScenarioForPosition(session, 1);
   ...
   const nextPosition = month + 1;
const nextScenario = nextPosition <= 12 ? getScenarioForPosition(session, nextPosition) : null;

    // Update session state
    session.month = nextScenario ? nextScenario.month : 13;
    session.financials = updatedFinancials;
    session.history.push({ month, choiceId: option.id, tag: option.tag, netWorth: updatedFinancials.netWorth, prevFinancials: prev });
    sessions.set(sessionId, session);
...
// 3b. POST /api/simulation/undo — revert to the previous month's decision point
app.post('/api/simulation/undo', (req, res) => {
  try {
    ...
    if (!session.history.length) {
      return res.status(400).json({ error: "There's no previous decision to go back to." });
    }

    const lastEntry = session.history.pop();
    session.financials = lastEntry.prevFinancials;
    session.month = lastEntry.month;
    sessions.set(sessionId, session);

    const currentScenario = getScenarioForPosition(session, session.month);
    ...
    console.error("Error in /api/simulation/undo:", err);
    res.status(500).json({ error: "Something went wrong undoing that decision." });
  }
});
